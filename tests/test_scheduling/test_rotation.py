...
async def test_simplest_rotation_path():
    ...
    # === 執行 rotation ===
    paths = list(scheduling._origin_rotation(a1, max_depth=5))
...
async def test_no_cycle_when_teacher_busy():
    ...
    paths = list(scheduling._origin_rotation(a1))
...
async def test_basic_cycle():
    ...
    # === 執行 rotation ===
    cycles = list(scheduling._origin_rotation(a1))
...
async def test_long_cycle_max_depth():
    ...
    # === 執行 rotation ===
    cycles = list(scheduling._origin_rotation(a1, max_depth=3))  # 設定最大深度為 3
...
async def test_yan_young_jing_2_4():
    """測試顏永進老師的 2-4兩截的 課程配置
    """    
    cycles = await scheduling.rotation("顏永進", weekday=2, period=4, max_depth=2, refresh=False)
    cycles_list = list(cycles)
